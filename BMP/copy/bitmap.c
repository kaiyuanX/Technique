...
/* the position of the file to be opened */
ClImage *clLoadImage(char *path)
{
	ClImage *bmpImg;
	FILE *pFile; //file to be loaded
	...
	if(bmpImg==NULL)
	{
		printf("failing apply memory...");
		return NULL;
	}
	pFile = fopen(path, "rb");
	if (!pFile)
	{
		printf("Failing loading...");
		free(bmpImg);
		return NULL;
	}
	...
	if (fileType == 0x4D42)
	{
		...
		else if (bmpInfoHeader.biBitCount == 24)
		{
			...
			bmpImg->height = height;
			bmpImg->channels = 3;
			bmpImg->imageData = (unsigned char *)malloc(sizeof(unsigned char) * width * height * 3);

			/* step is the number of bytes of a row */
			step = channels * width;

			/* offset is the number of pads */
			offset = (channels * width) % 4;
			if (offset != 0)
			{
				offset = 4 - offset;
...
/* bmpImg is the data adress the of file to be saved */
